...
The objective of this case is to get you understand logistic regression (binary classification) and some important ideas such as cut-off probability, symmetric and asymmetric misclassification rates (cost), which are important as an input argument for cross validation. 
...
<<<<<<< HEAD
1. Symmetric cost function (misclassification rate) with 1:1 cost ratio, or equivalently pcut=1/2. This is not very appropriate for cases such as Credit Card Default:

```{r}
#Symmetric cost and equivalently pcut=1/2 
cost1 <- function(r, pi){
  pcut <- 1/2 #equivalant 1:1 symmetric cost ratio
  mean(((r==0)&(pi>pcut)) | ((r==1)&(pi<pcut)))
}
cost1(r = credit_train$default, pi = pred_glm0_train)
=======
1. Symmetric cost function (misclassification rate) with 1:1 cost ration, or equivalently pcut=1/2. This is not very appropriate for cases such as Credit Card default.  

```{r}
#Symmetric cost, with pcut in the function input argument
cost1 <- function(r, pi, pcut){
  mean(((r==0)&(pi>pcut)) | ((r==1)&(pi<pcut)))
}

#Symmetric cost, with pcut NOT in the function input argument
pcut <- 1/2 #prespecify pcut value
cost1 <- function(r, pi){
  mean(((r==0)&(pi>pcut)) | ((r==1)&(pi<pcut)))
}
```
2. Asymmetric cost function (misclassification rate) with 5:1 cost ration, or equivalently pcut=1/(5+1).

```{r}
#Asymmetric cost, with pcut in the function input argument
...
#Asymmetric cost, with pcut NOT in the function input argument (This will be used for cross validation for Credit Card default case.)
pcut <- 1/(5+1) #prespecify pcut value, cost ratio 5:1
cost2 <- function(r, pi){
  weight1 <- 5
  weight0 <- 1
  c1 <- (r==1)&(pi<pcut) #logical vector - true if actual 1 but predict 0
...
>>>>>>> cc5d310e7a195c7f6c1586445ab8f438b6ed7548
```

2. Asymmetric cost function (misclassification rate) with 5:1 cost ratio, and equivalently pcut=1/(5+1).This is more appropriate for cases such as Credit Card Default:
...
costfunc  <- function(r, pi){
    ...
    pcut <- 1/(1+weight1/weight0)  # equivalant 5:1 asymmetric cost ratio
    c1 <- (r==1)&(pi < pcut)    # count for "true=1 but pred=0" (FN)
    c0 <- (r==0)&(pi >= pcut)   # count for "true=0 but pred=1" (FP)
    cost <- mean(weight1*c1 + weight0*c0)  # misclassification with weight
    return(cost) # you have to return to a value when you write R functions
} # end 

#Asymmetric cost
costfunc(r = credit_train$default, pi = pred_glm0_train)
=======
...
#Asymmetric cost
cost2(r = credit_train$default, pi = pred_glm0_train)
>>>>>>> cc5d310e7a195c7f6c1586445ab8f438b6ed7548
```
